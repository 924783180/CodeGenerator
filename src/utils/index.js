 ...
 * @param key {String} 设置浏览器cookie的键
 * @param value {String} 设置浏览器cookie的值
 * @param expires {String} 设置浏览器cookie的有效期（天）
 * @param path {String} 设置浏览器cookie的地址
 * @return {void}
 * @example setCookie(token, 'asdlkjfalksj')
 */
export function setCookie(key, value, expires, path = "/") {
  let date = new Date();
  date.setDate(Number(date.getDate() + expires));
  let cookie = "";
...
/**
 * @description 删除浏览器cookie
 * @param key {String} 设置浏览器cookie的键
 * @param path {String} 设置浏览器cookie的地址
 */
export function removeCookie(key, path = "/") {
  document.cookie = key + "=;expires=" + (new Date(0)).toGMTString() + "; path=" + path;
}
...
function formatStr(str, isUnderlineToHump) {
  //let str = data;
  if (isUnderlineToHump) {
    for (let i = 0, len = str.length; i < len; i++) {
      if ("_" === str[i]) {
        let item = (i === str.length - 1) ? "" : str[i + 1];
        str = str.replace(str[i] + item, item.toUpperCase());
        i--;
      }
    }
  } else {
...
function formatObj(data, isUnderlineToHump) {
  if (typeOf(data) === "Array") {
    let arr = [];
    for (let i = 0, len = data.length; i < len; i++) {
      arr.push(formatObj(data[i], isUnderlineToHump));
    }
    return arr;
  }
  if (typeOf(data) === "Object") {
    let obj = {};
    for (let key in data) {
      let field = formatStr(key, isUnderlineToHump);
      let item = data[key];
      if (typeOf(data) === "Array" || typeOf(data) === "Object") {
        item = formatObj(item, isUnderlineToHump);
      }
      obj[field] = item;
    }
    return obj;
 ...
 * @param {Object} data 要转化的数据
 */
export function humpToUnderline(data) {
  if (typeOf(data) === "String") {
    return formatObj(data, false);
  } else {
    return formatObj(data, false);
  }
}
...
export function listToTree(list) {
  ...
  let tempArr = [];
  for (let key in list) {
    tempArr[list[key].id] = list[key];
  }
  for (let key in tempArr) {
    let id = tempArr[key].id;
    let pid = tempArr[key].pId;
    if (!tempArr[pid]) {
      arr.push(tempArr[key]);
    } else {
      if (!tempArr[pid].children) {
        tempArr[pid]["children"] = [];
      }
      if (!tempArr[pid].children[id]) {
        tempArr[pid].children.push(tempArr[key]);
      }
    }
  }
  return arr;
